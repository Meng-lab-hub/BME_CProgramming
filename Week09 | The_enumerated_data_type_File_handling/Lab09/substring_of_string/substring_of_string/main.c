...
int main(void) {
    ...
    char* modified = substring_copy(string, 3, 9);
    printf("Old string: %s\n", string);
    printf("New string: %s\n", modified);
    free(modified);
    modified = NULL;
...
char* substring_copy(char* str, int starting, long ending) {
    ...
    char* subs = (char*)malloc((ending - starting + 1) * sizeof(char));
    for (int i = 0, j = starting; i < ending - starting; i++, j++)
        subs[i] = str[j];
    subs[ending - starting] = '\0';
    return subs;
}
...
// then we initialize a brandnew string with just as long as we need, all we need is the space for a substring between starting index and ending index
